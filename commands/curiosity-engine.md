...
```bash
phone-a-friend --to <BACKEND> --repo "$PWD" --sandbox read-only --fast --prompt "<relay-prompt>"
```
...
**Binary mode** (`RELAY_MODE = binary`):
```bash
phone-a-friend --to <BACKEND> --repo "$PWD" --sandbox read-only --fast --prompt "<re-prompt>"
```
