...
function run(argv) {
  ...
  program2.command("install").description("Install Claude plugin").option("--claude", "Install for Claude", false).option("--all", "Alias for --claude", false).option("--mode <mode>", "Installation mode: symlink or copy", "symlink").option("--force", "Replace existing installation", false).option("--repo-root <path>", "Repository root path", repoRootDefault()).option("--no-claude-cli-sync", "Skip Claude CLI sync").action((opts) => {
  ...
  program2.command("update").description("Update Claude plugin (equivalent to install --force)").option("--mode <mode>", "Installation mode: symlink or copy", "symlink").option("--repo-root <path>", "Repository root path", repoRootDefault()).option("--no-claude-cli-sync", "Skip Claude CLI sync").action((opts) => {
    const lines = installHosts({
      repoRoot: opts.repoRoot,
      target: "claude",
      mode: opts.mode,
  ...
  program2.command("uninstall").description("Uninstall Claude plugin").option("--claude", "Uninstall for Claude", false).option("--all", "Alias for --claude", false).action((opts) => {
    const target = opts.all ? "all" : "claude";
    const lines = uninstallHosts({ target });
    for (const line of lines) console.log(line);
  });
  try {
    program2.parse(normalized, { from: "user" });
  } catch (err) {
